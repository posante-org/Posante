...
#define BOOST_TEST_MODULE Posante Test Suite
...
{
    ClearDatadirCache();
    pathTemp = GetTempPath() / strprintf("test_posante_%lu_%i", (unsigned long)GetTime(), (int)(InsecureRandRange(100000)));
...
CBlock TestChainSetup::CreateBlock(const std::vector<CMutableTransaction>& txns, const CScript& scriptPubKey)
{
    std::unique_ptr<CBlockTemplate> pblocktemplate = BlockAssembler(
        Params(), DEFAULT_PRINTPRIORITY)
                                                         .CreateNewBlock(scriptPubKey, nullptr, false);
