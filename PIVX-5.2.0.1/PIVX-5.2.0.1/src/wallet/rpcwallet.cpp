...
CTxDestination GetNewAddressFromLabel(const std::string purpose, const UniValue& params, const CChainParams::Base58Type addrType = CChainParams::PUBKEY_ADDRESS)
{
...
UniValue sethdseed(const JSONRPCRequest& request)
{
    ...
    CPubKey master_pub_key = request.params[1].isNull() ?
                                 ...
                                 parseWIFKey(request.params[1].get_str(), pwallet);
    ...
    SaplingScriptPubKeyMan* sspk_man = pwallet->CanSupportFeature(FEATURE_SAPLING) ?
                                           pwallet->GetSaplingScriptPubKeyMan() :
                                           nullptr;
    if (sspk_man) {
...
UniValue getnewaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            "getnewaddress ( \"label\" )\n"
            "\nReturns a new Posante address for receiving payments.\n"
            ...
            "\nResult:\n"
            "\"posanteaddress\"    (string) The new posante address\n"
...
UniValue getnewstakingaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            ...
            "\nReturns a new Posante cold staking address for receiving delegated cold stakes.\n"
    ...
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            ...
            HelpRequiringPassphrase() + "\n"
                                        ...
                                        "\nExamples:\n" +
            HelpExampleCli("getnewshieldaddress", "") + HelpExampleRpc("getnewshieldaddress", ""));
...
UniValue listshieldunspent(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 4)
        throw std::runtime_error(
            ...
            "\nExamples\n" +
            HelpExampleCli("listshieldunspent", "") + HelpExampleCli("listshieldunspent", "6 9999999 false \"[\\\"ptestsapling1h0w73csah2aq0a32h42kr7tq4htlt5wfn4ejxfnm56f6ehjvek7k4e244g6v8v3pgylmz5ea8jh\\\",\\\"ptestsapling1h0w73csah2aq0a32h42kr7tq4htlt5wfn4ejxfnm56f6ehjvek7k4e244g6v8v3pgylmz5ea8jh\\\"]\"") + HelpExampleRpc("listshieldunspent", "6 9999999 false \"[\\\"ptestsapling1h0w73csah2aq0a32h42kr7tq4htlt5wfn4ejxfnm56f6ehjvek7k4e244g6v8v3pgylmz5ea8jh\\\",\\\"ptestsapling1h0w73csah2aq0a32h42kr7tq4htlt5wfn4ejxfnm56f6ehjvek7k4e244g6v8v3pgylmz5ea8jh\\\"]\""));
...
UniValue delegatoradd(const JSONRPCRequest& request)
{
    ...
    if (!keyID)
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Unable to get KeyID from Posante address");
...
UniValue delegatorremove(const JSONRPCRequest& request)
{
    ...
    if (!pwalletMain->HasAddressBook(*keyID))
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Unable to get Posante address from addressBook");
...
{
    const CChainParams::Base58Type addrType = (AddressBook::IsColdStakingPurpose(strPurpose) ?
...
UniValue listdelegators(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            ...
            "   \"address\": \"xxx\",  (string) Posante address string\n"
            "   }\n"
            "  ...\n"
            "]\n"
...
UniValue listshieldaddresses(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            ...
            "\nExamples:\n" +
            HelpExampleCli("listshieldaddresses", "") + HelpExampleRpc("listshieldaddresses", ""));
...
UniValue getrawchangeaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            "getrawchangeaddress\n"
            "\nReturns a new Posante address, for receiving change.\n"
...
UniValue setlabel(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 2)
        throw std::runtime_error(
            "setlabel \"posanteaddress\" \"label\"\n"
            "\nSets the label associated with the given address.\n"

            "\nArguments:\n"
            "1. \"posanteaddress\"   (string, required) The posante address to be associated with a label.\n"
...
UniValue sendtoaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 4)
        throw std::runtime_error(
            "sendtoaddress \"posanteaddress\" amount ( \"comment\" \"comment-to\" )\n"
            "\nSend an amount to a given address. The amount is a real and is rounded to the nearest 0.00000001\n" +
            HelpRequiringPassphrase() + "\n"

                                        "\nArguments:\n"
                                        "1. \"posanteaddress\"  (string, required) The posante address to send to.\n"
                                        "2. \"amount\"      (numeric, required) The amount in POSA to send. eg 0.1\n"
...
UniValue CreateColdStakeDelegation(const UniValue& params, CTransactionRef& txNew, CReserveKey& reservekey)
{
    ...
    if (!IsValidDestination(stakeAddr) || !isStaking)
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Posante staking address");
    ...
    if (params.size() > 2 && !params[2].isNull() && !params[2].get_str().empty()) {
        ...
        if (!IsValidDestination(dest) || isStaking)
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Posante spending address");
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 7)
        throw std::runtime_error(
            ...
            HelpRequiringPassphrase() + "\n"
                                        ...
                                        "\nArguments:\n"
                                        "1. \"stakingaddress\"      (string, required) The posante staking address to delegate.\n"
                                        "2. \"amount\"              (numeric, required) The amount in POSA to delegate for staking. eg 100\n"
                                        "3. \"owneraddress\"        (string, optional) The posante address corresponding to the key that will be able to spend the stake.\n"
...
UniValue getshieldbalance(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 3)
        throw std::runtime_error(
            ...
            HelpExampleCli("getshieldbalance", "") + HelpExampleCli("getshieldbalance", "ptestsapling1h0w73csah2aq0a32h42kr7tq4htlt5wfn4ejxfnm56f6ehjvek7k4e244g6v8v3pgylmz5ea8jh") +
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            CURRENCY_UNIT + "\n"
                            ...
                            "      \"address\" : \"posanteaddress\",     (string) The Posante address involved in the transaction\n"
            ...
            CURRENCY_UNIT + "\n"
                            ...
                            "\nExamples:\n" +
            HelpExampleCli("viewshieldtransaction", "\"1075db55d416d3ca199f55b6084e2115b9345e16c5cf302fc80e9d5fbf5d48d\"") + HelpExampleRpc("viewshieldtransaction", "\"1075db55d416d3ca199f55b6084e2115b9345e16c5cf302fc80e9d5fbf5d48d\""));
...
UniValue listaddressgroupings(const JSONRPCRequest& request)
{
    if (request.fHelp)
        throw std::runtime_error(
            ...
            "  [\n"
            "    [\n"
            "      \"posanteaddress\",     (string) The posante address\n"
            "      amount,                 (numeric) The amount in POSA\n"
...
UniValue signmessage(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 2)
        throw std::runtime_error(
            "signmessage \"posanteaddress\" \"message\"\n"
            "\nSign a message with the private key of an address" +
            HelpRequiringPassphrase() + "\n"

                                        "\nArguments:\n"
                                        "1. \"posanteaddress\"  (string, required) The posante address to use for the private key.\n"
...
UniValue getreceivedbyaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            "getreceivedbyaddress \"posanteaddress\" ( minconf )\n"
            "\nReturns the total amount received by the given posanteaddress in transactions with at least minconf confirmations.\n"

            "\nArguments:\n"
            "1. \"posanteaddress\"  (string, required) The posante address for transactions.\n"
            "2. minconf             (numeric, optional, default=1) Only include transactions confirmed at least this many times.\n"

            "\nResult:\n"
            "amount   (numeric) The total amount in POSA received at this address.\n"
...
UniValue getreceivedbylabel(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "amount              (numeric) The total amount in POSA received for this label.\n"
...
UniValue getbalance(const JSONRPCRequest& request)
{
    if (request.fHelp || (request.params.size() > 4))
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "amount              (numeric) The total amount in POSA received for this wallet.\n"
    ...
    isminefilter filter = ISMINE_SPENDABLE | (fIncludeWatchOnly ?
                                                     (fIncludeShielded ? ISMINE_WATCH_ONLY_ALL : ISMINE_WATCH_ONLY) :
                                                     ISMINE_NO);
...
UniValue getcoldstakingbalance(const JSONRPCRequest& request)
{
    if (request.fHelp || (request.params.size() != 0))
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "amount              (numeric) The total amount in POSA received for this wallet in P2CS contracts.\n"
...
static UniValue legacy_sendmany(const UniValue& sendTo, int nMinDepth, std::string comment, bool fIncludeDelegated)
{
    ...
    for (const std::string& name_ : keys) {
        ...
        if (!IsValidDestination(dest) || isStaking)
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Posante address: ") + name_);
...
UniValue sendmany(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 5)
        throw std::runtime_error(
            ...
            "\nSend to multiple destinations. Recipients are transparent or shield Posante addresses.\n"
            ...
            HelpRequiringPassphrase() + "\n"
                                        ...
                                        "    {\n"
                                        "      \"address\":amount   (numeric) The posante address (either transparent or shield) is the key,\n"
                                        "                                     the numeric amount in POSA is the value\n"
                                        "      ,...\n"
                                        "    }\n"
...
UniValue addmultisigaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "Each key is a Posante address or hex-encoded public key.\n"
            "If 'label' is specified, assign address to that label.\n"

            "\nArguments:\n"
            "1. nrequired        (numeric, required) The number of required signatures out of the n keys or addresses.\n"
            "2. \"keysobject\"   (string, required) A json array of posante addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"address\"  (string) posante address or hex-encoded public key\n"
            "       ...,\n"
            "     ]\n"
            "3. \"label\"      (string, optional) A label to assign the addresses to.\n"

            "\nResult:\n"
            "\"posanteaddress\"  (string) A posante address associated with the keys.\n"
...
UniValue listreceivedbyaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 4)
        throw std::runtime_error(
            ...
            "    \"amount\" : x.xxx,                  (numeric) The total amount in POSA received by the address\n"
...
UniValue listreceivedbyshieldaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() == 0 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nExamples:\n" +
            HelpExampleCli("listreceivedbyshieldaddress", "\"ps1ra969yfhvhp73rw5ak2xvtcm9fkuqsnmad7qln79mphhdrst3lwu9vvv03yuyqlh42p42st47qd\"") + HelpExampleRpc("listreceivedbyshieldaddress", "\"ps1ra969yfhvhp73rw5ak2xvtcm9fkuqsnmad7qln79mphhdrst3lwu9vvv03yuyqlh42p42st47qd\""));
...
UniValue listtransactions(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 6) throw std::runtime_error(
        ...
        "[\n"
        "  {\n"
        "    \"address\":\"posanteaddress\",    (string) The posante address of the transaction.\n"
        "    \"category\":\"category\",      (string) The transaction category (send|receive|orphan|immature|generate).\n"
        "    \"amount\": x.xxx,          (numeric) The amount in POSA. This is negative for the 'send' category, and positive\n"
        "                                         for the 'receive' category,\n"
        "    \"vout\" : n,               (numeric) the vout value\n"
        "    \"fee\": x.xxx,             (numeric) The amount of the fee in POSA. This is negative and only available for the \n"
...
UniValue listsinceblock(const JSONRPCRequest& request)
{
    if (request.fHelp)
        throw std::runtime_error(
            ...
            "{\n"
            "  \"transactions\": [\n"
            "    \"address\":\"posanteaddress\",    (string) The posante address of the transaction. Not present for move transactions (category = move).\n"
            "    \"category\":\"send|receive\",     (string) The transaction category. 'send' has negative amounts, 'receive' has positive amounts.\n"
            "    \"amount\": x.xxx,          (numeric) The amount in POSA. This is negative for the 'send' category, and for the 'move' category for moves \n"
            "                                          outbound. It is positive for the 'receive' category, and for the 'move' category for inbound funds.\n"
            "    \"vout\" : n,               (numeric) the vout value\n"
            "    \"fee\": x.xxx,             (numeric) The amount of the fee in POSA. This is negative and only available for the 'send' category of transactions.\n"
...
UniValue gettransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  \"amount\" : x.xxx,        (numeric) The transaction amount in POSA\n"
            "  \"confirmations\" : n,     (numeric) The number of confirmations\n"
            "  \"bcconfirmations\" : n,             (numeric) DEPRECATED: Will be removed in a future release\n"
            ...
            "      \"amount\" : x.xxx                  (numeric) The amount in POSA\n"
            "      \"label\" : \"label\",              (string) A comment for the address/transaction, if any\n"
            "      \"vout\" : n,                       (numeric) the vout value\n"
            "    }\n"
            "    ,...\n"
...
UniValue walletpassphrase(const JSONRPCRequest& request)
{
    if (pwalletMain->IsCrypted() && (request.fHelp || request.params.size() < 2 || request.params.size() > 3))
        throw std::runtime_error(
            ...
            "This is needed prior to performing transactions related to private keys such as sending POSAs\n"
...
UniValue encryptwallet(const JSONRPCRequest& request)
{
    if (!pwalletMain->IsCrypted() && (request.fHelp || request.params.size() != 1))
        throw std::runtime_error(
            ...
            "\nNow set the passphrase to use the wallet, such as for signing or sending POSAs\n" +
            HelpExampleCli("walletpassphrase", "\"my pass phrase\"") +
            "\nNow we can so something like sign\n" +
            HelpExampleCli("signmessage", "\"posanteaddress\" \"test message\"") +
    ...
    StartShutdown();
    return "wallet encrypted; posante server stopping, restart to run with encrypted wallet. The keypool has been flushed, you need to make a new backup.";
}
...
UniValue listunspent(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 4)
        throw std::runtime_error(
            ...
            "1. minconf          (numeric, optional, default=1) The minimum confirmations to filter\n"
            "2. maxconf          (numeric, optional, default=9999999) The maximum confirmations to filter\n"
            "3. \"addresses\"    (string) A json array of posante addresses to filter\n"
            "    [\n"
            "      \"address\"   (string) posante address\n"
            "      ,...\n"
            "    ]\n"
            ...
            "    \"vout\" : n,               (numeric) the vout value\n"
            "    \"address\" : \"address\",  (string) the posante address\n"
    ...
    if (request.params.size() > 2) {
        ...
        for (unsigned int inx = 0; inx < inputs.size(); inx++) {
            ...
            if (!IsValidDestination(dest))
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Posante address: ") + input.get_str());
...
UniValue lockunspent(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "A locked transaction output will not be chosen by automatic coin selection, when spending POSAs.\n"
...
UniValue settxfee(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. amount         (numeric, required) The transaction fee in POSA/kB rounded to the nearest 0.00000001\n"
...
UniValue getwalletinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,                      (numeric) the total POSA balance of the wallet (cold balance excluded)\n"
            "  \"delegated_balance\": xxxxx,              (numeric) the POSA balance held in P2CS (cold staking) contracts\n"
            "  \"cold_staking_balance\": xx,              (numeric) the POSA balance held in cold staking addresses\n"
            "  \"unconfirmed_balance\": xxx,              (numeric) the total unconfirmed balance of the wallet in POSA\n"
            "  \"immature_delegated_balance\": xxxxxx,    (numeric) the delegated immature balance of the wallet in POSA\n"
            "  \"immature_cold_staking_balance\": xxxxxx, (numeric) the cold-staking immature balance of the wallet in POSA\n"
            "  \"immature_balance\": xxxxxx,              (numeric) the total immature balance of the wallet in POSA\n"
            ...
            "  \"paytxfee\": x.xxxx                       (numeric) the transaction fee configuration, set in POSA/kB\n"
...
UniValue getstakingstatus(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"staking_enabled\": true|false,     (boolean) whether staking is enabled/disabled in posante.conf\n"
            "  \"coldstaking_enabled\": true|false, (boolean) whether cold-staking is enabled/disabled in posante.conf\n"
            "  \"haveconnections\": true|false,     (boolean) whether network connections are present\n"
            "  \"mnsync\": true|false,              (boolean) whether the required communitynode/spork data is synced\n"
            "  \"walletunlocked\": true|false,      (boolean) whether the wallet is unlocked\n"
            "  \"stakeablecoins\": n                (numeric) number of stakeable UTXOs\n"
            "  \"stakingbalance\": d                (numeric) POSA value of the stakeable coins (minus reserve balance, if any)\n"
...
UniValue setstakesplitthreshold(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "coinstake transaction will have 4 outputs (of 500 POSA each)." +
            HelpRequiringPassphrase() + "\n"

                                        "\nArguments:\n"
                                        "1. value                   (numeric, required) Threshold value (in POSA).\n"
...
UniValue autocombinerewards(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || (fEnable && request.params.size() != 2) || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nWallet will automatically monitor for any coins with value below the threshold amount, and combine them if they reside with the same Posante address\n"
...
UniValue printMultiSend()
{
    ...
    act.pushKV("MultiSendCommunitynode Activated?", pwalletMain->fMultiSendMasternodeReward);
    ret.push_back(act);
...
UniValue multisend(const JSONRPCRequest& request)
{
    ...
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid POSA address");
...
UniValue getsaplingnotescount(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            ...
            "\nExamples:\n" +
            HelpExampleCli("getsaplingnotescount", "0") + HelpExampleRpc("getsaplingnotescount", "0"));
