...
#include "qt/posante/splash.h"
#include "qt/posante/welcomecontentwidget.h"
...
{
    return QCoreApplication::translate("posante-core", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in posante.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in posante.qrc)
...
/** Class encapsulating Posante Core startup and shutdown.
...
/** Main Posante application object */
...
public:
    ...
    /// Get window identifier of QMainWindow (PosanteGUI)
...
{
    window = new PosanteGUI(networkStyle, 0);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &PosanteGUI::detectShutdown);
}
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &PosanteGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (pwalletMain) {
            ...
            window->addWallet(PosanteGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(PosanteGUI::DEFAULT_WALLET);
        ...
        // Posante: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &PosanteGUI::handlePaymentRequest);
        connect(window, &PosanteGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(0, "Runaway exception", QObject::tr("A fatal error occurred. Posante can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
int main(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(posante_locale);
    Q_INIT_RESOURCE(posante);
    ...
    /// 6. Determine availability of data directory and parse posante.conf
    /// - Do not call GetDataDir(true) before this step finishes
    if (!fs::is_directory(GetDataDir(false))) {
        QMessageBox::critical(0, QObject::tr("Posante Core"),
    ...
    try {
        ...
        QMessageBox::critical(0, QObject::tr("Posante Core"), QObject::tr("Error: %1").arg(e.what()));
        return 1;
    }
#ifdef ENABLE_WALLET
    ...
    if (!masternodeConfig.read(strErr)) {
        QMessageBox::critical(0, QObject::tr("Posante Core"),
            QObject::tr("Error reading communitynode configuration file: %1").arg(strErr.c_str()));
        return 0;
    }
    ...
    // posante: links repeatedly have their payment requests routed to this process:
