...
#include "qt/posante/sendcustomfeedialog.h"
#include "guiutil.h"
#include "optionsmodel.h"
#include "qt/posante/forms/ui_sendcustomfeedialog.h"
#include "qt/posante/qtutils.h"
#include "walletmodel.h"
#include <QComboBox>
#include <QListView>

SendCustomFeeDialog::SendCustomFeeDialog(PosanteGUI* parent, WalletModel* model) : FocusedDialog(parent),
...
void SendCustomFeeDialog::accept()
{
    ...
    if (customFee >= insaneFee) {
        ...
        inform(tr("Fee too high. Must be below: %1").arg(BitcoinUnits::formatWithUnit(walletModel->getOptionsModel()->getDisplayUnit(), insaneFee)));
        ...
        ui->lineEditCustomFee->setText(BitcoinUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), nFee));
        inform(tr("Fee too low. Must be at least: %1").arg(BitcoinUnits::formatWithUnit(walletModel->getOptionsModel()->getDisplayUnit(), CWallet::GetRequiredFee(1000))));
    } else {
...
CFeeRate SendCustomFeeDialog::getFeeRate()
{
    return ui->checkBoxRecommended->isChecked() ?
               feeRate :
               CFeeRate(GUIUtil::parseValue(ui->lineEditCustomFee->text(), walletModel->getOptionsModel()->getDisplayUnit()));
}
