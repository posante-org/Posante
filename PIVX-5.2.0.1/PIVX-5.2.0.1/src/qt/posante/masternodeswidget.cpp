...
#include "qt/posante/masternodeswidget.h"
#include "qt/posante/forms/ui_masternodeswidget.h"

#include "qt/posante/masternodewizarddialog.h"
#include "qt/posante/mninfodialog.h"
#include "qt/posante/mnrow.h"
...
#include "qt/posante/optionbutton.h"
#include "sync.h"
#include "util.h"
...
MasterNodesWidget::MasterNodesWidget(PosanteGUI* parent) : PWidget(parent),
                                                           ...
                                                           isLoading(false)
{
    ...
    /* Containers */
    setCssProperty(ui->left, "bg-trans-dark");
    ui->left->setContentsMargins(0, 20, 0, 20);
    setCssProperty(ui->right, "bg-trans-dark");
    ...
    /* Options */
    ui->btnAbout->setTitleClassAndText("btn-title-grey", tr("What is a Communitynode?"));
    ui->btnAbout->setSubTitleClassAndText("text-subtitle", tr("FAQ explaining what Communitynodes are"));
    ui->btnAboutController->setTitleClassAndText("btn-title-grey", tr("What is a Controller?"));
    ui->btnAboutController->setSubTitleClassAndText("text-subtitle", tr("FAQ explaining what is a Communitynode Controller"));

    setCssProperty(ui->listMn, "bg-trans");
...
void MasterNodesWidget::onEditMNClicked()
{
    if (walletModel) {
        ...
        if (index.sibling(index.row(), MNModel::WAS_COLLATERAL_ACCEPTED).data(Qt::DisplayRole).toBool()) {
            ...
            if (ask(tr("Start Communitynode"), tr("Are you sure you want to start communitynode %1?\n").arg(strAlias))) {
                WalletModel::UnlockContext ctx(walletModel->requestUnlock());
                if (!ctx.isValid()) {
                    // Unlock wallet was cancelled
                    inform(tr("Cannot edit communitynode, wallet locked"));
                    return;
                }
                startAlias(strAlias);
            }
        } else {
            inform(tr("Cannot start communitynode, the collateral transaction has not been confirmed by the network yet.\n"
                      "Please wait few more minutes (communitynode collaterals require %1 confirmations).")
...
void MasterNodesWidget::onStartAllClicked(int type)
{
    ...
    if (isLoading) {
        ...
        if (!pctx->isValid()) {
            warn(tr("Start ALL communitynodes failed"), tr("Wallet unlock cancelled"));
            return;
        }
        isLoading = true;
        if (!execute(type, std::move(pctx))) {
            isLoading = false;
            inform(tr("Cannot perform Communitynodes start"));
        }
    }
}
...
bool MasterNodesWidget::startAll(QString& failText, bool onlyMissing)
{
    ...
    if (amountOfMnFailed > 0) {
        failText = tr("%1 Communitynodes failed to start, %2 started").arg(amountOfMnFailed).arg(amountOfMnStarted);
        return false;
    }
    return true;
}
...
void MasterNodesWidget::run(int type)
{
    ...
    if (type == REQUEST_START_ALL || isStartMissing) {
        QString failText;
        QString inform = startAll(failText, isStartMissing) ? tr("All Communitynodes started!") : failText;
...
void MasterNodesWidget::onError(QString error, int type)
{
    if (type == REQUEST_START_ALL) {
        QMetaObject::invokeMethod(this, "inform", Qt::QueuedConnection,
            Q_ARG(QString, "Error starting all Communitynodes"));
    }
}
...
void MasterNodesWidget::onInfoMNClicked()
{
    ...
    if (!ctx.isValid()) {
        // Unlock wallet was cancelled
        inform(tr("Cannot show Communitynode information, wallet locked"));
        return;
    }
    showHideOp(true);
    ...
    if (dialog->exportMN) {
        if (ask(tr("Remote Communitynode Data"),
                tr("You are just about to export the required data to run a Communitynode\non a remote server to your clipboard.\n\n\n"
                   "You will only have to paste the data in the posante.conf file\nof your remote server and start it, "
                   "then start the Communitynode using\nthis controller wallet (select the Communitynode in the list and press \"start\").\n"))) {
            // export data
            QString exportedMN = "communitynode=1\n"
                                 "externalip=" +
                                 address.left(address.lastIndexOf(":")) + "\n" +
                                 "communitynodeaddr=" + address + +"\n" +
                                 "communitynodeprivkey=" + index.sibling(index.row(), MNModel::PRIV_KEY).data(Qt::DisplayRole).toString() + "\n";
            GUIUtil::setClipboard(exportedMN);
            inform(tr("Communitynode data copied to the clipboard."));
        }
    }
...
void MasterNodesWidget::onDeleteMNClicked()
{
    ...
    if (!ask(tr("Delete Communitynode"), tr("You are just about to delete Communitynode:\n%1\n\nAre you sure?").arg(qAliasString)))
        return;

    std::string strConfFile = "communitynode.conf";
    std::string strDataDir = GetDataDir().string();
    fs::path conf_file_path(strConfFile);
    if (strConfFile != conf_file_path.filename().string()) {
        throw std::runtime_error(strprintf(_("communitynode.conf %s resides outside data directory %s"), strConfFile, strDataDir));
    }
    ...
    if (fs::exists(pathBootstrap)) {
        ...
        if (!streamConfig.good()) {
            inform(tr("Invalid communitynode.conf file"));
            return;
        }
        ...
        for (std::string line; std::getline(streamConfig, line); linenumber++) {
            ...
            if (!(iss >> alias >> ip >> privKey >> txHash >> outputIndex)) {
                ...
                if (!(iss >> alias >> ip >> privKey >> txHash >> outputIndex)) {
                    streamConfig.close();
                    inform(tr("Error parsing communitynode.conf file"));
                    return;
                }
            }
        ...
        if (lineCopy.size() == 0) {
            lineCopy = "# Communitynode config file\n"
        ...
        if (lineNumToRemove != -1) {
            fs::path pathConfigFile = AbsPathForConfigVal(fs::path("communitynode_temp.conf"));
            FILE* configFile = fsbridge::fopen(pathConfigFile, "w");
            fwrite(lineCopy.c_str(), std::strlen(lineCopy.c_str()), 1, configFile);
            fclose(configFile);

            fs::path pathOldConfFile = AbsPathForConfigVal(fs::path("old_communitynode.conf"));
            if (fs::exists(pathOldConfFile)) {
                fs::remove(pathOldConfFile);
            }
            rename(pathMasternodeConfigFile, pathOldConfFile);

            fs::path pathNewConfFile = AbsPathForConfigVal(fs::path("communitynode.conf"));
        ...
        inform(tr("communitynode.conf file doesn't exists"));
    }
}
...
void MasterNodesWidget::onCreateMNClicked()
{
    ...
    if (!ctx.isValid()) {
        // Unlock wallet was cancelled
        inform(tr("Cannot create Communitynode controller, wallet locked"));
        return;
    }

    if (walletModel->getBalance() <= (COIN * 10000)) {
        inform(tr("Not enough balance to create a communitynode, 1,00,00,000 %1 required.").arg(CURRENCY_UNIT.c_str()));
        return;
    }
    showHideOp(true);
    ...
    if (openDialogWithOpaqueBackgroundY(dialog, window, 5, 7)) {
        if (dialog->isOk) {
            ...
            warn(tr("Error creating communitynode"), dialog->returnStr);
        }
    }
    dialog->deleteLater();
}
