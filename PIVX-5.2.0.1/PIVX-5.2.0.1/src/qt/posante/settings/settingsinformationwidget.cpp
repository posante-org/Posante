...
#include "qt/posante/settings/forms/ui_settingsinformationwidget.h"
...
SettingsInformationWidget::SettingsInformationWidget(PosanteGUI* _window, QWidget* parent) : PWidget(_window, parent),
                                                                                             ui(new Ui::SettingsInformationWidget)
{
    ...
    setCssProperty({ui->labelTitleDataDir,
    ...
    connect(ui->pushButtonFile, &QPushButton::clicked, [this]() {
        if (!GUIUtil::openConfigfile())
            inform(tr("Unable to open posante.conf with default application"));
    });
...
void SettingsInformationWidget::run(int type)
{
    if (type == REQUEST_UPDATE_COUNTS) {
        QMetaObject::invokeMethod(this, "setCommunitynodeCount",
