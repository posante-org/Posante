...
#include "qt/posante/addnewcontactdialog.h"
#include "qt/posante/forms/ui_send.h"
#include "qt/posante/guitransactionsutils.h"
#include "qt/posante/loadingdialog.h"
#include "qt/posante/optionbutton.h"
#include "qt/posante/qtutils.h"
#include "qt/posante/sendchangeaddressdialog.h"
#include "qt/posante/sendconfirmdialog.h"
#include "script/standard.h"

#define REQUEST_PREPARE_TX 1
#define REQUEST_REFRESH_BALANCE 2

SendWidget::SendWidget(PosanteGUI* parent) : PWidget(parent),
                                             ui(new Ui::send),
                                             coinIcon(new QPushButton())
{
    ...
    setCssProperty(coinIcon, "coin-icon-posa");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this]() { onPOSASelected(true); });
    connect(ui->pushRight, &QPushButton::clicked, [this]() { onPOSASelected(false); });
...
void SendWidget::showHideCheckBoxDelegations(CAmount delegationBalance)
{
    ...
    // coincontrol is not selected, and we are trying to spend transparent POSAs.
    ...
    if (showCheckBox)
        ui->checkBoxDelegations->setToolTip(
            tr("Possibly spend coins delegated for cold-staking (currently available: %1").arg(GUIUtil::formatBalance(delegationBalance, nDisplayUnit)));
}
...
void SendWidget::ProcessSend(QList<SendCoinsRecipient>& recipients, bool hasShieldedOutput, const std::function<bool(QList<SendCoinsRecipient>&)>& func)
{
...
void SendWidget::onCoinControlClicked()
{
    if (walletModel->getBalance() > 0) {
        const interfaces::WalletBalances& newBalance = walletModel->GetWalletBalances();
...
void SendWidget::onShieldCoinsClicked()
{
    ...
    if (availableBalance > 0) {
        ...
        ProcessSend(recipients, true, [this](QList<SendCoinsRecipient>& recipients) {
            ...
            if (!res.result) {
                inform(tr("Error generating address to shield POSAs"));
                return false;
            }
        ...
        inform(tr("You don't have any transparent POSAs to shield."));
    }
}
...
void SendWidget::onPOSASelected(bool _isTransparent)
{
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        auto posaAdd = Standard::DecodeDestination(address.toStdString(), isStakingAddr);

        if (!Standard::IsValidDestination(posaAdd) || isStakingAddr) {
            inform(tr("Invalid address"));
            return;
        }

        if (walletModel->isMine(posaAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(posaAdd, dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
