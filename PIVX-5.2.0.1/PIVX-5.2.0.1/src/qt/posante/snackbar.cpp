...
#include "qt/posante/forms/ui_snackbar.h"
#include "qt/posante/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(PosanteGUI* _window, QWidget* parent) : QDialog(parent),
                                                           ui(new Ui::SnackBar),
                                                           window(_window),
                                                           timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &PosanteGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
