...
#include "qt/posante/addresseswidget.h"
#include "qt/posante/addnewaddressdialog.h"
#include "qt/posante/addresslabelrow.h"
#include "qt/posante/forms/ui_addresseswidget.h"
...
AddressesWidget::AddressesWidget(PosanteGUI* parent) : PWidget(parent),
                                                       ui(new Ui::AddressesWidget)
{
    ...
    setCssProperty(ui->listAddresses, "bg-trans");
...
void AddressesWidget::onStoreContactClicked()
{
    if (walletModel) {
        ...
        auto posaAdd = Standard::DecodeDestination(address.toUtf8().constData(), isStakingAddress);

        if (!Standard::IsValidDestination(posaAdd)) {
        ...
        bool isShielded = walletModel->IsShieldedDestination(posaAdd);
        if (walletModel->updateAddressBookLabels(posaAdd, label.toUtf8().constData(),
                isShielded       ? AddressBook::AddressBookPurpose::SHIELDED_SEND :
                isStakingAddress ? AddressBook::AddressBookPurpose::COLD_STAKING_SEND :
