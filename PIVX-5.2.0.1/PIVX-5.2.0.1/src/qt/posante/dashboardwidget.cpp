...
#include "qt/posante/dashboardwidget.h"
#include "clientmodel.h"
#include "guiutil.h"
#include "optionsmodel.h"
#include "qt/posante/forms/ui_dashboardwidget.h"
#include "qt/posante/qtutils.h"
#include "qt/posante/sendconfirmdialog.h"
#include "qt/posante/txrow.h"
#include "utiltime.h"
...
DashboardWidget::DashboardWidget(PosanteGUI* parent) : PWidget(parent),
                                                       ui(new Ui::DashboardWidget)
{
    ...
    // Containers
    setCssProperty({this, ui->left}, "bg-trans-dark");
    ...
    setCssProperty(ui->labelSquarePosa, "square-chart-posa");
    setCssProperty(ui->labelPosa, "text-chart-posa");
    ...
    setCssProperty(ui->labelAmountPosa, "text-stake-posa-disable");
    ...
    // Transactions
    setCssProperty(ui->listTransactions, "bg-trans");
    ...
    setCssProperty(ui->chartContainer, "bg-trans");
    ...
    if (window)
        connect(window, &PosanteGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    ...
    // update the display unit, to not use the default ("POSA")
    updateDisplayUnit();
}
...
void DashboardWidget::changeChartColors()
{
    ...
    if (isLightTheme()) {
        ...
        gridLineColorX = QColor(74, 144, 144);
        linePenColorY = gridLineColorX;
        backgroundColor = linePenColorY;
    }
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair POSA
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            posa = (pair.first != 0) ? pair.first / 100000000 : 0;
            chartData->totalPosa += pair.first;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesPosa.append(posa);

        int max = std::max(posa, posa);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->append(chartData->valuesPosa);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalPosa > 0) {
        setCssProperty(ui->labelAmountPosa, "text-stake-posa");
    } else {
        setCssProperty(ui->labelAmountPosa, "text-stake-posa-disable");
    }
    forceUpdateStyle({ui->labelAmountPosa});
    ui->labelAmountPosa->setText(GUIUtil::formatBalance(chartData->totalPosa, nDisplayUnit));
