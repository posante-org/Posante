...
#include "guiutil.h"
#include "qt/posante/forms/ui_sendconfirmdialog.h"
...
TxDetailDialog::TxDetailDialog(QWidget* parent, bool _isConfirmDialog, const QString& warningStr) : FocusedDialog(parent),
    ...
    if (_tx) {
        ...
        if (_tx->tx->vout.size() == 1 || (_tx->tx->sapData && _tx->tx->sapData->vShieldedOutput.size() == 1)) {
            ...
            ui->textSendLabel->setText(QString::number(_tx->tx->vout.size() +
                                                       (_tx->tx->sapData ? _tx->tx->sapData->vShieldedOutput.size() : 0)) +
                                       " recipients");
        }
