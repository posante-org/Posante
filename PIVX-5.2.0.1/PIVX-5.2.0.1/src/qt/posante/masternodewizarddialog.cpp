...
#include "qt/posante/forms/ui_masternodewizarddialog.h"

#include "activemasternode.h"
#include "optionsmodel.h"
#include "pairresult.h"
#include "qt/posante/guitransactionsutils.h"
#include "qt/posante/mnmodel.h"
...
MasterNodeWizardDialog::MasterNodeWizardDialog(WalletModel* model, QWidget* parent) : FocusedDialog(parent),
...
bool MasterNodeWizardDialog::createMN()
{
    ...
    if (!walletModel->getMNCollateralCandidate(collateralOut)) {
        ...
        if (prepareStatus.status != WalletModel::OK) {
            returnStr = tr("Prepare community node failed.\n\n%1\n").arg(returnMsg);
            return false;
        }
    ...
    if (!fs::exists(pathBootstrap)) {
        returnStr = tr("communitynode.conf file doesn't exists");
        return false;
    }
    ...
    if (!streamConfig.good()) {
        returnStr = tr("Invalid communitynode.conf file");
        return false;
    }
    ...
    for (std::string line; std::getline(streamConfig, line); linenumber++) {
        ...
        if (!(iss >> alias >> ip >> privKey >> txHash >> outputIndex)) {
            ...
            if (!(iss >> alias >> ip >> privKey >> txHash >> outputIndex)) {
                streamConfig.close();
                returnStr = tr("Error parsing communitynode.conf file");
                return false;
            }
        }
        lineCopy += line + "\n";
    ...
    returnStr = tr("Community node created! Wait %1 confirmations before starting it.").arg(MasternodeCollateralMinConf());
    return true;
}
