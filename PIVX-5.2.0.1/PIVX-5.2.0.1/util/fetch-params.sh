...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/PosanteParams"
    else
        PARAMS_DIR="$HOME/.posante-params"
    fi
fi
...
Failed to fetch the Posante zkSNARK parameters!
...
function main() {
    ...
    cat <<EOF
Posante - fetch-params.sh

This script will fetch the Posante zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common Posante zkSNARK parameters. Note that it is
